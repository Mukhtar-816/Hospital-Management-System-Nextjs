  ...
  if (!isOnline && !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
        ...
        <p className="text-textMuted max-w-md">Please check your internet connection to access patient records and your schedule.</p>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between bg-surface p-6 rounded-2xl border border-border shadow-sm">
        <div>
          <h1 className="text-3xl font-bold text-text tracking-tight">Clinical Workspace</h1>
          <p className="text-textMuted mt-1">
            Welcome, Dr. {data?.doctor?.fullname || "Provider"}. You have {data?.stats?.today_appointments || 0} patients scheduled today.
          </p>
        </div>
        {!isOnline && (
          <Badge variant="error" className="px-4 py-1.5 animate-pulse">Connection Lost</Badge>
        )}
      </div>
        ...
        >
          {nextPatient ? (
            <div className="flex flex-col md:flex-row items-center justify-between p-8 bg-gradient-to-r from-primary/10 to-transparent rounded-2xl border border-primary/20 gap-8">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 bg-primary text-white rounded-2xl flex items-center justify-center font-black text-3xl shadow-xl shadow-primary/30">
                  {nextPatient.patientname?.split(' ').map((n: any) => n[0]).join('') || <User size={32} />}
                </div>
                <div>
                  <h4 className="text-2xl font-bold text-text">{nextPatient.patientname || "Patient Unknown"}</h4>
                  <p className="text-sm text-textMuted flex items-center gap-2 mt-1">
                    <Badge variant="info" className="rounded-md">{nextPatient.gender || "N/A"}</Badge>
                    <span className="opacity-50">•</span>
                    <span className="font-medium">{nextPatient.type || "General Consultation"}</span>
                  </p>
                  <div className="flex gap-2 mt-4">
                    {nextPatient.status === 'in_progress' ? (
                      <Badge variant="success" className="px-3 py-1 animate-pulse">In Progress</Badge>
                    ) : (
                      <Badge variant="warning" className="px-3 py-1">Ready for Consultation</Badge>
                    )}
                  </div>
                </div>
              </div>
              <div className="text-center md:text-right flex flex-col items-center md:items-end gap-2">
                <div className="flex items-center gap-2 text-primary">
                  ...
                  <p className="text-4xl font-black">
                    {nextPatient.starttime ? new Date(nextPatient.starttime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "--:--"}
                  </p>
                </div>
                <p className="text-xs font-bold text-textMuted uppercase tracking-widest mb-4">Target Consultation Time</p>
                <Button
                  size="lg"
                ...
                >
                  {nextPatient.status === 'in_progress' ? "Continue Session" : "Start Consultation"}
                  <ArrowRight size={18} />
                </Button>
              </div>
            </div>
            ...
            <div className="py-16 text-center text-textMuted border-2 border-dashed border-border rounded-2xl bg-border/5">
              ...
              <h4 className="text-lg font-bold text-text">No pending patients</h4>
              <p className="max-w-xs mx-auto mt-1">All currently scheduled appointments have been processed. Time for a well-deserved break!</p>
            </div>
          )}
        </Card>
      </div>
